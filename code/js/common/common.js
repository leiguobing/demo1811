...
function aotu(banner,aapan) {
	var banner = getid('banner');
	var lis = banner.getElementsByTagName('li');    
	var spa = banner.getElementsByTagName('span');
	var btn = banner.getElementsByClassName('btn')[0];
	var lef = banner.getElementsByClassName('lef')[0];
	var righ = banner.getElementsByClassName('righ')[0];

	/*
		1.先将所有的li(图片的宽度)的宽度获取
		2.用添样式的方式在将图片移到右边，并将第一张留在中间;
 
	*/
	var iw = lis[0].offsetWidth//图片的宽度
	//将图片移动到右侧
	...
	//将第一张图片放到中间
	lis[0].style.left = 0
	var num = 0;  //代表图片(li)的下标
	var timer = null;
	function text() {  
		//定时器开启时图片往左边移动
		startMove(lis[num], { 'left': -iw });
		//判断当图片移动到达临界值的时候则num下标返回为0.否则继续累加
		num = ++num >= lis.length ? 0 : num;
		// console.log(num)
		//将移动到左边的图片快速移动到右边
		lis[num].style.left = iw + 'px';
		//当num为0后在将图片往可视区移动，一直循环下去
		startMove(lis[num], { 'left': 0 });
		spas()       
	}
	//焦点跟随，循环span(小圆点)的个数，拿到图片的下标作为span的下标
	function spas() {
		for (var i = 0; i < spa.length; i++){
			spa[i].className = '';
		}
		spa[num].className = aapan;
	}    
	timer = setInterval(text, 5000)    
	//鼠标移入可视区定时器关闭
	...
	for (var i = 0; i < spa.length; i++) {
		...
		spa[i].onclick = function () {
			//当图片下标num小于圆点下标的时候旧图移动到左边，新图从右边出来
			if (num < this.index) {
				startMove(lis[num], { 'left': -iw });
				lis[this.index].style.left = iw +'px'
			}
			//当图片下标num大于于圆点下标的时候旧图移动到右边，新图从左边边出来
			if (num > this.index) {
				startMove(lis[num], { 'left': iw });
				lis[this.index].style.left = -iw +'px'
			}
